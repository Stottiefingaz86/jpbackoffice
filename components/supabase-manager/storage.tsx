        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {buckets.map((bucket: any) => (
            <Tooltip key={bucket.id}>
              <TooltipTrigger render={<Button key={bucket.id} variant="outline" className="flex-row justify-start text-left h-auto p-4 gap-4" />}><Folder className="h-4 w-4 text-muted-foreground" /><div className="flex-1">
                                      <h2 className="font-semibold mb-1">{bucket.name}</h2>

                                      <p className="text-xs text-muted-foreground">
                                        Updated {new Date(bucket.updated_at).toLocaleDateString()}
                                      </p>
                                    </div><Badge variant={bucket.public ? 'default' : 'secondary'}>
